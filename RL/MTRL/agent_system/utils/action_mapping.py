...
LastEditTime: 2025-04-29 02:55:45
'''
...
def get_special_token(model_id:str = '/nfs-shared/models/llama-3', bases:list = [10,3,3,3,2,2,2,2,2,2,11,11]) -> list:  #假设永远不会出现8641这个数
    '''
    bases: button+camera
...
def prepare_for_remap_control_token(tokenizer_type:str = "llama-2",bases:list = [10,3,3,3,2,2,2,2,2,2,21,21],not_text=True):
...
class ActionTokenizer(ABC):
    ...
    def __init__(self):
        self._env_null_action = {
            'hotbar.1': False, 'hotbar.2': False, 'hotbar.3': False, 'hotbar.4': False, 'hotbar.5': False,
            'hotbar.6': False, 'hotbar.7': False, 'hotbar.8': False, 'hotbar.9': False,
            'forward': False, 'back': False, 'left': False, 'right': False, 'sprint': False, 'sneak': False,
            'use': False, 'drop': False, 'attack': False, 'jump': False, 'inventory': False,
            'camera': [0.0, 0.0]
        }
...
class TextActionTokenizer(ActionTokenizer):
    env_action_text_action_dict = {
        'hotbar.1': "keyboard.1", 'hotbar.2': "keyboard.2", 'hotbar.3': "keyboard.3", 'hotbar.4': "keyboard.4", 'hotbar.5': "keyboard.5",
        'hotbar.6': "keyboard.6", 'hotbar.7': "keyboard.7", 'hotbar.8': "keyboard.8", 'hotbar.9': "keyboard.9",
        'forward': 'keyboard.w', 'back': 'keyboard.s', 'left': 'keyboard.a', 'right': 'keyboard.d', 
        'sprint': 'keyboard.left.ctrl', 'sneak': 'keyboard.left.shift', 'jump': 'keyboard.space', 
        'use': 'click(button=left)', 'drop': 'keyboard.q', 'attack': 'click(button=right)', 'inventory': 'keyboard.e',
    }
    text_action_env_action_dict = {
        "keyboard.1": 'hotbar.1', "keyboard.2": 'hotbar.2', "keyboard.3": 'hotbar.3', "keyboard.4": 'hotbar.4', "keyboard.5": 'hotbar.5',
        "keyboard.6": 'hotbar.6', "keyboard.7": 'hotbar.7', "keyboard.8": 'hotbar.8', "keyboard.9": 'hotbar.9',
        "keyboard.w": 'forward', "keyboard.s": 'back', "keyboard.a": 'left', "keyboard.d": 'right',
        "keyboard.left.ctrl": 'sprint', "keyboard.left.shift": 'sneak', "keyboard.space": 'jump',
        "click(button=left)": 'use', "keyboard.q": 'drop', "click(button=right)": 'attack', "keyboard.e": 'inventory',
    }
    camera_re = re.compile(r"move\(dx=([-+]?\d*\.?\d+),dy=([-+]?\d*\.?\d+)\)")
    def __init__(self, action_chunk_len:int=0):
        super().__init__()
        self.act_beg_token = "<action>"
        self.act_end_token = "</action>"
        self.action_re = re.compile(rf"{self.act_beg_token}(.*?){self.act_end_token}", flags=re.DOTALL)
        self.action_chunk_len = action_chunk_len

    def encode(self, trajectory: Dict):
        encoded_trajectory = []
        ...
        last_id = frame_ids[0]-1
        acc_ids = []
        for idx in range(traj_len):
            acc_ids.append(idx)
            if len(acc_ids) >= self.action_chunk_len or last_id + 1 != frame_ids[idx] or idx + 1 == traj_len:

                raw_actions = [ trajectory['actions'][acc_idx] for acc_idx in acc_ids]
                encoded_action = " ".join([f"{self.act_beg_token} {self.map_env_action_text_action(env_action=raw_action)} {self.act_end_token}" for raw_action in raw_actions])
                encoded_trajectory.append({
                    "action":encoded_action,
                    "raw_action": raw_actions,
                    "observations": [observations[acc_idx] for acc_idx in acc_ids], #不过最后可能就用一张
                    'interact_id':interact_ids[acc_ids[0]],
                    'frames':(frame_ids[acc_ids[0]], len(acc_ids), frame_ids[acc_ids[-1]]),
                })
                acc_ids = []
            last_id = frame_ids[idx]
            
        return encoded_trajectory
    
    def decode(self, tokens: str):
        matches = self.action_re.findall(tokens)
        env_actions = [self.remap_text_action_env_action(match) for match in matches]
        if not env_actions:
            env_actions = [self.env_null_action.copy()]
        return env_actions

    def map_env_action_text_action(self, env_action:dict):
        text_action_list = []
        if self.env_null_action[self.camera] != list(env_action[self.camera]):
            y = env_action[self.camera][0] / CAMERA_SCALER
            x = env_action[self.camera][1] / CAMERA_SCALER
            camera_text = f"move(dx={x},dy={y})"
            text_action_list.append(camera_text)
        for button in self.buttons:
            if button not in env_action:
                continue
            if self.env_null_action[button] != env_action[button]:
                text_action_list.append(self.env_action_text_action_dict[button])
        text_action = "\n ".join(text_action_list)
        if not text_action:
            text_action = "no-op"
        return text_action
    
    def remap_text_action_env_action(self,text_action:str):
        
        env_action = self.env_null_action.copy()
        if "no-op" in text_action:
            return env_action
        
        match = self.camera_re.search(text_action)
        if match:
            dx = float(match.group(1))
            dy = float(match.group(2))
            env_action["camera"] = [dy * CAMERA_SCALER, dx * CAMERA_SCALER]
            
        for ta in text_action.splitlines():
            clean_ta = ta.strip()
            if not clean_ta:
                continue
            env_a = self.text_action_env_action_dict.get(clean_ta)
            if env_a:
                env_action[env_a] = True
        return env_action
          
    @property
    def encode_null_action(self,):
        return f"{self.act_beg_token} no-op {self.act_end_token}"
...
class ReservedActionTokenizer(ActionTokenizer):
    def __init__(self, tokenizer_type="llama-3",
        ...
        self.n_camera_bins = self.camera_config.n_camera_bins

        self.action_transformer = ActionTransformer(**self.camera_config.action_transformer_kwargs)
        self.action_mapper = CameraHierarchicalMapping(n_camera_bins=self.camera_config.n_camera_bins)
...
class OneActionTokenizer(ReservedActionTokenizer):
    ...
    def __init__(self,tokenizer_type="llama-2",bases=[10,3,3,3,2,2,2,2,2,2,21,21],
                 camera_quantization_scheme="mu_law",
                 camera_mu=20,
                 camera_binsize=1):
    ...
    def encode(self,trajectory:dict) -> list[tuple[int]]:
        ...
        interact_ids = trajectory.get('interact_ids',[""]*traj_len)
        minerl_action_transformed = {key: np.array(val) for key, val in minerl_actions.items() if key in Buttons.ALL or key == "camera"} #从list到ndarray
...
class BPEActionTokenizer(ReservedActionTokenizer):
    ...
    def interact_ids2token(self,interact_ids:list)->str:
        zero_include_token_list = [map_control_token( action_idx, 0,self.tokenizer_type) for action_idx in interact_ids]
...
if __name__ == '__main__':
    ...
    def raw_str2env(reserved_tokens_str):
        ...
        sim = MinecraftSim(camera_config=tokenizer.camera_config)
