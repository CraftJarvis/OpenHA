...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if "rollout_log_probs" in batch.batch:
        ...
        # 额外的 valid mask：过滤掉 log_probs == -1 的 token
        valid_lp_mask = (log_probs != -1)

        # 最终有效位置 = response_mask & valid_lp_mask
        final_mask = mask.bool() & valid_lp_mask

        # 避免除以 0
        if final_mask.sum() == 0:
            ppl = torch.tensor(float("inf"))
        else:
            nll = -log_probs[final_mask]
            mean_nll = nll.mean()
    ...
    metrics = {
        ...
        "critic/rewards/ppl": ppl.detach().item(), #hkc
        # reward
