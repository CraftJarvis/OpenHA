...
def single_step_camera_reward(
    gt: torch.Tensor,          # (..., 2)
    pred: torch.Tensor,        # (..., 2)
    scales=(3.0, 4.5),         # 归一化尺度，来自你数据的大致幅度
    deadzone=0.05,             # 死区阈值（原始量纲）
    success_tau=0.15,          # 成功阈值（归一化后的 L2）
    base=1.0,                  # 基准偏置 c
    bonus=0.3,                 # 命中奖励 b
    fp_lambda=0.5              # 假阳性惩罚系数 λ_fp
):
    scales = torch.as_tensor(scales, dtype=gt.dtype, device=gt.device)
    e = (pred - gt) / scales   # 归一化误差
    # Smooth L1（Huber，beta=1）
    hub = F.smooth_l1_loss(e, torch.zeros_like(e), reduction='none').sum(dim=-1)  # shape (...)

    # 成功奖励：归一化 L2 < tau
    succ = (torch.norm(e, p=2, dim=-1) < success_tau).to(gt.dtype) * bonus

    # 假阳性惩罚：gt≈0 却输出了明显动作（按原始量纲判断更直观）
    gt_small = (gt.abs() < deadzone)          # (..., 2) bool
    pred_big = (pred.abs() >= deadzone)       # (..., 2) bool
    gt_big = (gt.abs()>=deadzone)
    pred_small = (pred.abs()<= deadzone)

    #(gt_small & pred_big).to(gt.dtype).sum(dim=-1) * fp_lambda  \
    fp_pen = (gt_big & pred_small).to(gt.dtype).sum(dim=-1) * fp_lambda

    # 最终奖励
    r = base - hub + succ - fp_pen
...
def compute_score(data_source, solution_str, ground_truth, extra_info=None):
    ...
    try:
        ground_action = tokenizer.decode(ground_truth)[0]
        solution_action = tokenizer.decode(solution_str)[0]

        if ground_action["attack"] == 1:
            breakpoint()
        if random.random() < 0.001:
            print(f"[DEBUG] ground_action: {ground_action}\n[DEBUG] solution_action: {solution_action}")
    ...
    button_score = 5*correct_buttons / (num_buttons + len(solution_action) - 1) if num_buttons > 0 else 0.0
    ...
    try:
        ...
        camera_score = 2*single_step_camera_reward(camera_gt, camera_pred)
    ...
    #print("len(solution):", len(solution_str))
    final_score = button_score  + camera_score - len(solution_str)*0.00001 #0.001 not ok

    return {"score": final_score, "button_score": button_score, "camera_score": camera_score, "penalty": len(solution_str)*0.00001}
